...
// Polyglot Zobrist keys - these are the standard values used by Polyglot opening books.
// The keys are generated using a specific PRNG seeded with a known value.
// Total: 781 keys = 768 (pieces) + 4 (castling) + 8 (en passant) + 1 (side)

// polyglotRandom contains the 781 pseudo-random numbers used by Polyglot.
// These are the official values from the Polyglot source code.
...
// Piece indices in Polyglot format:
// BlackPawn=0, WhitePawn=1, BlackKnight=2, WhiteKnight=3, etc.
// Order: Pawn, Knight, Bishop, Rook, Queen, King (black first, then white)
func polyglotPieceIndex(piece board.Piece, isWhite bool) int {
	// Polyglot order: bp, wp, bn, wn, bb, wb, br, wr, bq, wq, bk, wk
	base := 0
	switch piece {
	case board.Pawn:
		base = 0
...
// PolyglotHash computes the Polyglot Zobrist hash for a position.
// This must match exactly with the hash used in Polyglot .bin files.
func PolyglotHash(pos board.Position) uint64 {
	var hash uint64

	// Hash pieces (indices 0-767)
	// Polyglot square mapping: a1=0, b1=1, ..., h8=63 (same as our mapping)
	allPieces := pos.Pawns | pos.Knights | pos.Bishops | pos.Rooks | pos.Queens | pos.Kings
	for sq := 0; sq < 64; sq++ {
		sqBB := board.Bitboard(1 << sq)
		if allPieces&sqBB == 0 {
			continue
		}

		isWhite := pos.White&sqBB != 0
		...
		pieceIdx := polyglotPieceIndex(piece, isWhite)
		hash ^= polyglotRandom[64*pieceIdx+sq]
	}
	...
	// Only hash if there's actually a pawn that can capture
	if pos.EnPassant != 0 {
		epSq := bits.TrailingZeros64(uint64(pos.EnPassant))
		file := epSq & 7

		// Check if en passant is actually possible (pawn can capture)
		canCapture := false
		if pos.WhiteMove {
			// White to move, check if white pawn can capture
			// White pawns that could capture are on rank 5 (indices 32-39)
			if file > 0 {
				leftPawn := board.Bitboard(1 << (epSq - 9))
				if pos.Pawns&pos.White&leftPawn != 0 {
					canCapture = true
				}
			}
			if file < 7 {
				rightPawn := board.Bitboard(1 << (epSq - 7))
				if pos.Pawns&pos.White&rightPawn != 0 {
					canCapture = true
				}
			}
		} else {
			// Black to move, check if black pawn can capture
			if file > 0 {
				leftPawn := board.Bitboard(1 << (epSq + 7))
				if pos.Pawns&pos.Black&leftPawn != 0 {
					canCapture = true
				}
			}
			if file < 7 {
				rightPawn := board.Bitboard(1 << (epSq + 9))
				if pos.Pawns&pos.Black&rightPawn != 0 {
					canCapture = true
				}
			}
		}
